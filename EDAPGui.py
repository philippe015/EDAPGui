...
self.gui_loaded = False
    ...
    # Ajout du bouton Fleet Carrier Assist
    self.checkboxvar['Fleet Carrier Assist'] = BooleanVar()
    self.lab_ck['Fleet Carrier Assist'] = Checkbutton(
        root,
...
def check_cb(self, field):
    if field == 'Fleet Carrier Assist':
        if self.checkboxvar['Fleet Carrier Assist'].get() == 1 and not self.fc_assist.is_running():
